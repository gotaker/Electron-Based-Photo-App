...
The icon (`icon.svg`) is a vector-first design — a 6-blade camera aperture on a deep blue gradient squircle, matching the app's `#007AFF`-family palette. It is pre-exported at all required sizes:

| File | Use |
|------|-----|
| `icon.icns` | macOS (.dmg build) |
| `icon.ico` | Windows (.exe / NSIS) |
| `icon.png` | Linux (AppImage / deb), Electron fallback |
| `icon_512.png` … `icon_16.png` | Individual sizes for tray, taskbar, notifications |
| `icon.svg` | Master source — edit this to restyle the icon |

electron-builder will pick the right format automatically via `"icon": "icon"` in `package.json`.
